...
DispenseableBlock::DispenseableBlock(const std::string& name, int blockId):
...
void DispenseableBlock::onRedstoneUpdate(BlockSource& region, const BlockPos& pos, int strength, bool active) {
	if(active)
	{
		...
		if(dispenseAtLocation(getDispenseType(testStone), region, pos, testStone))
		{
			playDispenseEffects(region.getLevel(), pos);
		}
	}
}
...
void DispenseableBlock::playDispenseEffects(Level* level, const BlockPos& pos) {
